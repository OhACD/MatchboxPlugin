...
This checklist defines the minimum bar for calling a Matchbox update production ready.
...
Run these before calling the release production ready:
...
- Paper API changes or deprecations in supported Minecraft versions
- Behavior that only appears on a real server and not in mocked tests
- Performance regressions during repeated rounds or many concurrent sessions
- Extension API breakage for integrators using session builders or custom handlers
