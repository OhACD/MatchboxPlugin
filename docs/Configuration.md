...
Matchbox now separates server tuning from map geometry.
...
Use this for server-level gameplay tuning:
...
If you have old seat/spawn config in `plugins/Matchbox/config.yml`:

1. Enter the target map world.
